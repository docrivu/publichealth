...
if ($user['role'] === 'super_admin') {
    $totals = db()->query('SELECT (SELECT COUNT(*) FROM bht_assessments) AS bht_assessments, (SELECT COUNT(*) FROM bht_users WHERE role = "collector") AS collectors')->fetch();
    $stmt = db()->query(
        'SELECT a.*, u.full_name AS collector_name
         FROM bht_assessments a
         INNER JOIN bht_users u ON u.id = a.collector_id
         ORDER BY a.updated_at DESC'
    );
} else {
    $totalsStmt = db()->prepare('SELECT COUNT(*) AS bht_assessments FROM bht_assessments WHERE collector_id = :collector_id');
...
app_layout('Dashboard', function () use ($user, $totals, $assessments, $modules): void {
    ...
    <section class="grid dashboard-stats">
        <article class="card stat-card"><span>Assessments</span><strong><?= e((string) ($totals['bht_assessments'] ?? 0)) ?></strong></article>
